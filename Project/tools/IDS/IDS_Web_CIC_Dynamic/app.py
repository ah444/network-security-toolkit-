...
def configure():
    ...
    lr_model = LogisticRegression(max_iter=1000, random_state=42, class_weight=class_weight_param)
    rf_model = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1, class_weight=class_weight_param)

    use_svm = True
    if len(df) > LARGE_DATASET_THRESHOLD:
        use_svm = False
        logging.info("Large dataset detected; using K-Nearest Neighbors instead of SVM.")
        knn_model = KNeighborsClassifier(n_neighbors=5, n_jobs=-1)
    else:
        svm_model = LinearSVC(random_state=42, max_iter=1000)  # Removed n_jobs parameter
        ...
        try:
            ...
            try:
                ...
                if hasattr(model, 'predict_proba'):
                    ...
                    if n_classes > 2:
                        auc_scores["validation"][name] = roc_auc_score(y_val, val_proba, multi_class='ovr', average='weighted')
                        auc_scores["test"][name] = roc_auc_score(y_test, test_proba, multi_class='ovr', average='weighted')
                    else:
                    ...
                    n_classes = len(np.unique(y_train))
                    if n_classes > 2:
                        auc_scores["validation"][name] = roc_auc_score(y_val, val_decision, multi_class='ovr', average='weighted')
                        auc_scores["test"][name] = roc_auc_score(y_test, test_decision, multi_class='ovr', average='weighted')
                    else:
    ...
    threads = []
    threads.append(threading.Thread(target=train_and_eval, args=(lr_model, True, "logistic_regression")))
    threads.append(threading.Thread(target=train_and_eval, args=(rf_model, False, "random_forest")))
    if use_svm:
        threads.append(threading.Thread(target=train_and_eval, args=(svm_model, True, "svm")))
    else:
        threads.append(threading.Thread(target=train_and_eval, args=(knn_model, True, "knn")))
    for t in threads:
        t.start()
    for t in threads:
        t.join()
    ...
    models_trained = {"logistic_regression": lr_model, "random_forest": rf_model}
    if use_svm:
...
def predict_endpoint():
    ...
    if algo not in model_config["models"]:
        return jsonify({"error": "Invalid algorithm selection. Choose from: logistic_regression, random_forest, svm, knn."}), 400
    ...
    if algo in ["logistic_regression", "svm", "knn"]:
        try:
