...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (document.getElementById("uploadForm")) {
    ...
    // Function to load available models for selection
    ...
    configForm.addEventListener("submit", function(e) {
      ...
      // Get selected models from dropdown
      ...
      if (selectedModels.length === 0) {
        configResult.innerHTML = "<span style='color: #dc3545;'>❌ Please select at least one model to train.</span>";
        return;
      }
      ...
      .then(result => {
        if (result.message) {
          ...
          Object.entries(result.accuracies.validation).forEach(([algo, acc]) => {
            let algoName = modelNames[algo] || algo;
